...
static char *getColor(char *logType)
{
	char *lColor;
...
void log_display(char program[])
{
	...
	if (strcmp(program, "") != 0) {
		char buffer[100];
		sprintf(buffer, "SELECT * FROM liblog WHERE PROG_NAME LIKE '%s'", program);
		sql_stmt = buffer;
	}
	...
	while (sqlite3_step(stmt) == SQLITE_ROW) {
		printf("\n%s %s %s %s %s %s", getColor((char *)sqlite3_column_text(stmt, 0)),
		       sqlite3_column_text(stmt, 0), LNORM, sqlite3_column_text(stmt, 1),
...
static const char *getLogType(enum log_type logT)
{
	switch (logT) {
	case INFO:
		return "INFO";
...
void log_log(enum log_type logT, char message[])
{
	...
	snprintf(sql, sizeof(sql),
		 "INSERT INTO LIBLOG(LOG_TYPE,LOG_TIME,PROG_NAME, LOG) VALUES (?1,?2,?3,?4);");
